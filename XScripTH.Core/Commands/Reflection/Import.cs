...
[NoRuntimeInvocation]
public sealed class Import(ICommandRegistrar registrar) : ICommand, ICompileTimePhase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var rawPath = arguments is [CommandValueArgument { Value: string value }] ? value : null;
        return string.IsNullOrWhiteSpace(rawPath)
            ? throw new ArgumentException("Import path must be a non-empty string.", nameof(arguments))
