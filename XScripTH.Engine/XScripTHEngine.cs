...
public sealed class XScripTHEngine : ICommandExecutor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var index = 0; index < invocation.Arguments.Count; index++)
        {
            ...
            var resolved =
                await ResolveArgumentAsync(invocation.Arguments[index], expectedInputType, executionContext,
        ...
        CancellationToken cancellationToken)
    {
        switch (argument)
        {
            ...
            case CommandVariableArgument variableArgument:
                ...
                return !executionContext.TryGetVariable(variableArgument.Name, out var value)
                    ? throw new InvalidOperationException($"Variable '${variableArgument.Name}' has not been assigned.")
                    : new ArgumentResolution(value, null);

            case CommandInvocationArgument invocationArgument:
                var nestedOutput =
                    await ExecuteInvocationAsync(invocationArgument.Invocation, executionContext, cancellationToken)
                        .ConfigureAwait(false);
                return nestedOutput.Status == CommandStatus.Error
                    ? new ArgumentResolution(null, nestedOutput)
                    : new ArgumentResolution(RequireSingleOutputValue(nestedOutput, "Nested command"), null);

            case CommandBlockArgument blockArgument:
                return await ResolveBlockArgumentAsync(blockArgument, expectedInputType, executionContext,
                    cancellationToken).ConfigureAwait(false);
            ...
            case CommandFunctionReferenceArgument functionReferenceArgument:
                if (!executionContext.TryGetFunction(functionReferenceArgument.Name, out var block) || block is null)
                    throw new InvalidOperationException(
                        $"Function '@{functionReferenceArgument.Name}' has not been assigned.");

                return await ResolveBlockArgumentAsync(block, expectedInputType, executionContext, cancellationToken)
                    .ConfigureAwait(false);

            default:
                throw new InvalidOperationException(
                    $"Unsupported command argument type '{argument.GetType().FullName}'.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var output = await ExecuteAsync(block.Invocations, executionContext.CreateChildScope(), cancellationToken)
            .ConfigureAwait(false);
        return output.Status == CommandStatus.Error
            ? new ArgumentResolution(null, output)
            : new ArgumentResolution(RequireSingleOutputValue(output, "Command block"), null);
    }
